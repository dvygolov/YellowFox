...
public partial class ProfileEditorViewModel : ViewModelBase
{
    ...
    [ObservableProperty]
    private OsOption? _selectedOsOption;
    ...
    public ObservableCollection<OsOption> OsOptions { get; } = new(OsOption.Options);
    ...
    public ProfileEditorViewModel(DatabaseService databaseService, Profile? existingProfile, bool isCloneMode = false)
    {
        ...
        if (existingProfile != null)
        {
            ...
            SelectedOsOption = OsOptions.FirstOrDefault(option => option.Id == existingProfile.FingerprintConfig.Os)
                ?? OsOption.FromId(existingProfile.FingerprintConfig.Os);
        ...
        {
            // Default values
            SelectedOsOption = OsOption.FromId(GetDefaultOs());
...
public sealed class OsOption
{
    ...
    public string IconFill { get; }
    public string Background { get; }
    public string Border { get; }

    public static OsOption FromId(string? id)
    {
        var normalized = id?.Trim().ToLowerInvariant();
        return Options.FirstOrDefault(option => option.Id == normalized) ?? Options[0];
    }
}
